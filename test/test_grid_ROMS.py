...
def test_nofile():
    ...
    with pytest.raises(SystemExit):
        g = Grid(grid_file="does_not_exist.nc")


def test_subgrid():
    """Test subgrid specification"""
    i0, i1, j0, j1 = 20, 150, 30, 170
    g = Grid(grid_file=grid_file, subgrid=(i0, i1, j0, j1))
    assert g.xmin == i0
    assert g.xmax == i1 - 1
    assert g.ymin == j0
    assert g.ymax == j1 - 1
...
def test_subgrid_error():
    ...
    with pytest.raises(SystemExit):
        g = Grid(grid_file=grid_file, subgrid=(20, 150, 30, 222))
    with pytest.raises(SystemExit):
        g = Grid(grid_file=grid_file, subgrid=(1200, 20, 30, 180))
